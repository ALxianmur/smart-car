...
void TM4_Isr() interrupt 20 // 1Khz   1ms中断
{
    static uint32 count = 0;
    TIM4_CLEAR_FLAG; // 清除中断标志
    beep_handle();

    if (count % 20 == 0) // 20ms运行
    {
        key_timer_Handler(); // 按键定时中断服务 20ms
    }
    if (count % 10 == 0) // 10ms运行
    {
        key_Handle();
        /*传感器获得车辆和路面信息 */
        update_speed_and_distance();
        update_inductor_value();
        get_inductor_error(1.0, 0.17, 0, 0.17, 1.0, 0, 0);
    }
