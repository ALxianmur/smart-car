...
float raw_inductor_error1 = 0.0;
float inductor_error1 = 0.0;
float raw_inductor_error2 = 0.0;
float inductor_error2 = 0.0;
float raw_inductor_error3 = 0.0;
float inductor_error3 = 0.0;
...
uint16 get_adc(uint8 num)
{
    ...
    {
    case 1:
        adc_value = adc_once(ADC_P00, ADC_12BIT);
        break;
    case 2:
...
float get_inductor_error(float conf_level0, float conf_level1, float conf_level2, float conf_level3, float conf_level4, int using_cubic_fun, int using_5_induc)
{
    ...
    if (using_cubic_fun)
        err = (raw_err / 20) * ((raw_err * 3.125) * (raw_err * 0.005) + 4);
    else
        err = raw_err;
    inductor_error1 = err;
...
static void updata_element_value(void)
{
    ...
    horizon_diff = inductor_value_list[1] - inductor_value_list[3]; // 横电感差值
    slope_diff = inductor_value_list[0] - inductor_value_list[4];   // 斜电感差值
    horizon_sum = inductor_value_list[1] + inductor_value_list[3];  // 横电感和值
    slope_sum = inductor_value_list[0] + inductor_value_list[4];    // 斜电感和值

    horizon_left_error = inductor_value_list[1] - horizon_sum / 2;  // 直偏 左边横电感相对偏差 = 左横-（左横+右横）/2
    horizon_right_error = inductor_value_list[3] - horizon_sum / 2; // 右边横电感相对偏差 = 右横-（左横+右横）/2
    slope_left_error = inductor_value_list[0] - slope_sum / 2;      // 斜偏 左边斜电感相对偏差 = 左斜-（左斜+右斜）/2
    slope_right_error = inductor_value_list[4] - slope_sum / 2;     // 右边斜电感相对偏差 = 右斜-（左斜+右斜）/2
}
